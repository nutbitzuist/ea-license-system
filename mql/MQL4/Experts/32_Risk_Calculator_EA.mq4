...
void PlaceTrade(int orderType) { double price = (orderType == OP_BUY) ? Ask : Bid; double sl = (orderType == OP_BUY) ? price - StopLossPips * Point : price + StopLossPips * Point; double tp = (orderType == OP_BUY) ? price + TakeProfitPips * Point : price - TakeProfitPips * Point; OrderSend(Symbol(), orderType, g_calculatedLot, price, 10, sl, tp, "RiskCalc", 0, 0, clrNONE); }
//+------------------------------------------------------------------+
