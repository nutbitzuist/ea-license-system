  ...
  try {
    ...
    if (!apiKey || !apiSecret) {
      return NextResponse.json(
        { valid: false, message: "Missing credentials", errorCode: "INVALID_CREDENTIALS" },
        { status: 401 }
      )
    }
    ...
    if (!user || user.apiSecret !== apiSecret) {
      return NextResponse.json(
        { valid: false, message: "Invalid credentials", errorCode: "INVALID_CREDENTIALS" },
        { status: 401 }
      )
    }
    ...
    if (!mtAccount) {
      ...
      return NextResponse.json(
        { valid: false, message: "Account not registered", errorCode: "ACCOUNT_NOT_FOUND" },
        { status: 403 }
      )
    }
