...
input double   PinBarWickRatio = 0.6;    // Pin Bar Wick Ratio
input double   PinBarBodyRatio = 0.3;    // Pin Bar Body Ratio
input int      KeyLevelLookback = 20;    // Key Level Lookback
input double   LotSize = 0.1;            // Lot Size
input int      StopLoss = 100;           // Stop Loss (points)
input int      TakeProfit = 200;         // Take Profit (points)
input int      MagicNumber = 100013;     // Magic Number
...
int OnInit()
{
   ...
   Print("Price Action EA initialized and ready to trade");
   return INIT_SUCCEEDED;
}

void OnDeinit(const int reason)
{
   Print("Price Action EA stopped. Reason: ", reason);
}
...
void OpenOrder(int orderType)
{
   ...
   int ticket = OrderSend(Symbol(), orderType, tradeVolume, price, 10, sl, tp, "Price Action", MagicNumber, 0, clrNONE);
