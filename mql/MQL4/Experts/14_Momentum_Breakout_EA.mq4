...
input int      RangePeriod = 20;         // Range Period
...
input int      MagicNumber = 100014;     // Magic Number
...
int OnInit()
{
   Print("=== Momentum Breakout EA v1.0.0 ===");
   ...
   Print("Momentum Breakout EA initialized and ready to trade");
   return INIT_SUCCEEDED;
}

void OnDeinit(const int reason)
{
   Print("Momentum Breakout EA stopped. Reason: ", reason);
}
...
void OpenOrder(int orderType, double atrValue)
{
   ...
   int ticket = OrderSend(Symbol(), orderType, tradeVolume, price, 10, sl, 0, "Momentum BO", MagicNumber, 0, clrNONE);
