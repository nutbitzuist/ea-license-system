...
input int      Tenkan_Period = 9;        // Tenkan Period
input int      Kijun_Period = 26;        // Kijun Period
input int      Senkou_Period = 52;       // Senkou Period
...
int OnInit()
{
   Print("=== Ichimoku Cloud EA v1.0.0 ===");
   ...
   Print("Ichimoku Cloud EA initialized and ready to trade");
   return INIT_SUCCEEDED;
}

void OnDeinit(const int reason)
{
   Print("Ichimoku Cloud EA stopped. Reason: ", reason);
}
...
void OpenOrder(int orderType)
{
   ...
   int ticket = OrderSend(Symbol(), orderType, tradeVolume, price, 10, sl, tp, "Ichimoku Cloud", MagicNumber, 0, clrNONE);
