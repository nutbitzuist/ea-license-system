...
void OpenCopiedOrder(string sym, int type, double lot, double sl, double tp, int masterTicket) { double price = (type == OP_BUY) ? MarketInfo(sym, MODE_ASK) : MarketInfo(sym, MODE_BID); if(!CopySLTP) { sl = 0; tp = 0; } int ticket = OrderSend(sym, type, NormalizeDouble(lot, 2), price, 10, sl, tp, "MT" + IntegerToString(masterTicket), MagicNumber, 0, clrNONE); if(ticket < 0) Print("OrderSend failed: ", GetLastError()); }
void CloseMissingOrders(int &masterTickets[]) { for(int i = OrdersTotal() - 1; i >= 0; i--) { if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue; if(OrderMagicNumber() != MagicNumber) continue; bool found = false; for(int j = 0; j < ArraySize(masterTickets); j++) { if(StringFind(OrderComment(), "MT" + IntegerToString(masterTickets[j])) >= 0) { found = true; break; } } if(!found) if(!OrderClose(OrderTicket(), OrderLots(), (OrderType() == OP_BUY) ? Bid : Ask, 10, clrNONE)) Print("OrderClose failed: ", GetLastError()); } }
