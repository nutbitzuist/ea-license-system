...
input int      ADX_Threshold = 30;       // ADX Threshold
input int      ATR_Period = 14;          // ATR Period
input double   ATR_Multiplier = 2.0;     // ATR Multiplier
input double   LotSize = 0.1;            // Lot Size
input int      MagicNumber = 100020;     // Magic Number
...
int OnInit()
{
   ...
   Print("NOTE: Requires hedging-enabled broker");
   return INIT_SUCCEEDED;
}

void OnDeinit(const int reason) { Print("Hedge EA stopped. Reason: ", reason); }
...
void OnTick()
{
   ...
   if(buyCount == 0 && sellCount == 0) { OpenHedge(); return; }
   
   if(buyCount > 0 && sellCount > 0)
   {
      if(adx > ADX_Threshold)
      {
         if(plusDI > minusDI) { CloseOrder(sellTicket); Print("Uptrend confirmed, closed SELL hedge"); }
         else { CloseOrder(buyTicket); Print("Downtrend confirmed, closed BUY hedge"); }
      }
      return;
   }
...
void OpenHedge()
{
   ...
   {
      double atr = iATR(Symbol(), Period(), ATR_Period, 0);
      double riskSL = (atr * ATR_Multiplier) / Point; // Use ATR-based SL in points
      if(riskSL <= 0) riskSL = 100; // Default safety
      tradeVolume = GetLotSize(riskSL);
   }

   int buyTicket = OrderSend(Symbol(), OP_BUY, tradeVolume, Ask, 10, 0, 0, "Hedge BUY", MagicNumber, 0, clrNONE);
